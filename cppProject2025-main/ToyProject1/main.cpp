...
namespace ConsoleUtils
{
	...
	void InputPlayerKey(int& x, int& y)
	{
		if (_kbhit())
		{
			if(GetAsyncKeyState(VK_LEFT)) // 키보드 '<-' 눌렀을 때
			{
				x--;
				if (x < 0)
				{
...
//  shop, length << default가 아니기 때문에 앞부분 작성해줘야 되고, 
void ShowShop(Item shop[], int length, int x = 10, int y = 10)
{
	...
	for (int i = 0; i < length; i++)
	{
		...
		ConsoleUtils::GoToXY(x, y+1+i);	// 표의 틀보다 1칸 아래에 있어야 하기때문에 1을 더해줌. i만큼 더해준다.
...
int main()
{
	...
	while (true)
	{
		...
		std::cin >> inputNumber;   // 언제 주소 연산자 사용하고, 사용을 안하는데 이 부분이 잘 이해가 안갑니다.
		if (inputNumber == 1)
		{
			std::cout << "이름을 입력해주세요";
			std::cin >> name;
