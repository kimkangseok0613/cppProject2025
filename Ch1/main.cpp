...
* Q1. iostream 추가했는데 왜 "cout" 찾을 수 없는가?
* A. "cout" 특별한 공간에 존재하기 때문에 찾을 수 없다.
* Q2. 그 특별한 공간을 어떻게 사용해야 하는가?
* A. namespace에 대한 이해가 필요하다.
*/

// A회사 ACOM, B회사 BCOM
// ACOM에서 만든 goods, BCOM에서 만든 goods
// 문제점 : 특정 키워드의 이름들이 같으면 컴퓨터가 구변해서 판별할 수 없다. - 에러가 발생한다.
// ACOM - goods다.
// BCOM - goods다.
//
...
int main()
{
	...
	// 문제1. 숫자를 5개 입력받아서 A회사의 합계 저장을 한 후, 숫자를 5개 입력을 받아서 B회사의 합계에도 저장을 해주세요.
	...
	for (int i = 0;i < 5;i++)
	{
		std::cout << i + 1 << "번째 상품의 갯수를 입력해주세요." << std::endl;
		std::cin >> number;
	...
	// namespace를 게임에 적용하는 것
	// c버전 프로젝트 -> c++버전 프로젝트 변경
	// readme 오늘 날자 Change log
}